import logging
import numpy as np
from common import log_current_process_memory_usage
...
def allocate_memory():
    ...
    log_current_process_memory_usage("After allocating large list")

    # Stage 2: Allocate a large NumPy array
    large_array = np.zeros((1000, 1000))  # Approximately 8 MB
    log_current_process_memory_usage("After allocating large NumPy array")

    # Stage 3: Allocate another large list
    another_large_list = [1] * 10**7  # Approximately 80 MB
    log_current_process_memory_usage("After allocating another large list")

    # Stage 4: Allocate a large dictionary
    large_dict = {i: i for i in range(10**6)}  # Approximately 32 MB
    log_current_process_memory_usage("After allocating large dictionary")
